...
def extract_exporter_refs(directory):
    try:
        ...
        pdf_files = [file for file in os.listdir(
            directory) if file.endswith('.pdf')]
            ...
            for table in tables:
                ...
                if len(df) >= 8 and any(df.iloc[7].notna()):
                    ...
                    if 'Exporters Ref:' in exporter_ref:
                        exporter_ref = exporter_ref.replace(
                            'Exporters Ref:', '').replace('\n', ' ')
                        ...
                        if exporter_ref.strip().lower().startswith("s/c no :".lower()) or exporter_ref.strip().lower().startswith("S/C NO:".lower()):
                            # Strip "s/c no :" from the start
                            exporter_ref = exporter_ref.strip()[8:]
                        exporter_refs.append(exporter_ref.strip())
        return pd.DataFrame(exporter_refs, columns=['Exporters Ref'])
    except Exception as e:
        raise Exception(
            f'Error occurred while extracting exporter refs from {directory}: {str(e)}')
